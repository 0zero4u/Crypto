...
const PUBLIC_PORT = 8081; // Public WebSocket port for clients
const INTERNAL_LISTENER_PORT = 8082; // Internal KCP/UDP port for the listener
...
try {
    ...
    udpServer.on('message', (msg, rinfo) => {
        ...
        if (!kcpSession) {
            ...
            // CORRECTED: Add a buffer to the session to handle streaming data
            kcpSession.streamBuffer = '';
        ...
        // CORRECTED: Implement stream parsing logic
        let received;
        while ((received = kcpSession.recv())) {
            // Append new data to our session's buffer
            kcpSession.streamBuffer += received.toString();

            // Process the buffer to find complete, newline-delimited messages
            let newlineIndex;
            while ((newlineIndex = kcpSession.streamBuffer.indexOf('\n')) >= 0) {
                // Extract the complete message string before the newline
                const completeMessage = kcpSession.streamBuffer.substring(0, newlineIndex);

                // IMPORTANT: Remove the processed message (and the newline) from the buffer
                kcpSession.streamBuffer = kcpSession.streamBuffer.substring(newlineIndex + 1);

                if (completeMessage && wssAndroidClients?.clients.size > 0) {
                    wssAndroidClients.clients.forEach(client => {
                        if (client.readyState === WebSocket.OPEN) {
                            client.send(completeMessage); // Send the single, valid JSON string
                        }
                    });
                }
            }
    ...
    // CORRECT: Listen on all available network interfaces (0.0.0.0)
