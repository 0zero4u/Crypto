...
// Adjusted Heartbeat Frequency: Ping every 60 seconds. Client has 60 seconds to respond with a pong.
...
function attemptServerClosure(server, serverName, onDone) {
    if (!server) {
        // console.log(`[Receiver] PID: ${process.pid} --- ${serverName} was not initialized or already null. Skipping closure.`); // Reduced log
        onDone();
        return;
    }
...
try {
    ...
    wssAndroidClients.on('connection', (ws, req) => {
        ...
        ws.on('pong', () => {
            ws.isAlive = true;
            // console.log(`[Receiver] PID: ${process.pid} --- Pong from Android client ${ws.clientId}`); // Reduced log (very frequent)
        });
    ...
    wssListenerSource.on('connection', (wsListener, req) => {
        ...
        wsListener.on('message', (message) => {
            try {
                ...
                // const numAndroidClients = wssAndroidClients.clients.size; // Not strictly needed if just iterating
