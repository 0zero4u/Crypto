...
const MODEL_VERSION = "2.6-robust-hysteresis"; // Updated model version
...
const SCORE_CHANGE_THRESHOLD = 1.0; 
...
// --- NEW: WIDER and Corrected Hysteresis Thresholds for Bias ---
const BIAS_THRESHOLDS = {
    // To enter WEAK_BUY, score must cross 2.0. To exit, it must drop below 1.0.
    NEUTRAL_TO_WEAK_BUY: 2.0,
    WEAK_BUY_TO_NEUTRAL: 1.0,

    // To enter STRONG_BUY, score must cross 4.0. To exit, it must drop below 3.0.
    WEAK_BUY_TO_STRONG_BUY: 4.0,
    STRONG_BUY_TO_WEAK_BUY: 3.0,

    // Symmetrical thresholds for the sell side
    NEUTRAL_TO_WEAK_SELL: -2.0,
    WEAK_SELL_TO_NEUTRAL: -1.0,
    WEAK_SELL_TO_STRONG_SELL: -4.0,
    STRONG_SELL_TO_WEAK_SELL: -3.0,
};
...
// *** REVISED FUNCTION: Corrected Hysteresis with a Hierarchical State Machine ***
function getBiasFromScore(score, lastBias) {
    // Check for STRONG signal conditions first, this allows direct jumps to STRONG states
    if (score > BIAS_THRESHOLDS.WEAK_BUY_TO_STRONG_BUY) {
        return 'STRONG_BUY_BIAS';
    }
    if (score < BIAS_THRESHOLDS.WEAK_SELL_TO_STRONG_SELL) {
        return 'STRONG_SELL_BIAS';
    }

    // If coming from a STRONG state, check if we should downgrade
    if (lastBias === 'STRONG_BUY_BIAS' && score < BIAS_THRESHOLDS.STRONG_BUY_TO_WEAK_BUY) {
        return 'WEAK_BUY_BIAS';
    }
    if (lastBias === 'STRONG_SELL_BIAS' && score > BIAS_THRESHOLDS.STRONG_SELL_TO_WEAK_SELL) {
        return 'WEAK_SELL_BIAS';
    }

    // If currently in a STRONG state and not downgraded, stay there
    if (lastBias === 'STRONG_BUY_BIAS' || lastBias === 'STRONG_SELL_BIAS') {
        return lastBias;
    }

    // --- We are now only dealing with WEAK or NEUTRAL states ---

    // Check for entering WEAK signal conditions from a lower state
    if (score > BIAS_THRESHOLDS.NEUTRAL_TO_WEAK_BUY) {
        return 'WEAK_BUY_BIAS';
    }
    if (score < BIAS_THRESHOLDS.NEUTRAL_TO_WEAK_SELL) {
        return 'WEAK_SELL_BIAS';
    }
    
    // If coming from a WEAK state, check if we should downgrade to NEUTRAL
    if (lastBias === 'WEAK_BUY_BIAS' && score < BIAS_THRESHOLDS.WEAK_BUY_TO_NEUTRAL) {
        return 'NEUTRAL';
    }
    if (lastBias === 'WEAK_SELL_BIAS' && score > BIAS_THRESHOLDS.WEAK_SELL_TO_NEUTRAL) {
        return 'NEUTRAL';
    }
    
    // If currently in a WEAK state and not downgraded (and not upgraded to STRONG), stay there
    if (lastBias === 'WEAK_BUY_BIAS' || lastBias === 'WEAK_SELL_BIAS') {
        return lastBias;
    }
    
    // If none of the above conditions are met, the state is NEUTRAL
    return 'NEUTRAL';
}
...
function processTrade(data) {
    try {
        ...
        // Pass the last known bias to the updated function
