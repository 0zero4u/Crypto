...
function connectToSpot() {
    ...
    spotWsClient.on('message', (data) => {
        try {
            ...
            // Check for Pending Futures Confirmation
            if (pending_futures_confirmation && isSpotImbalanceFavorable(pending_futures_confirmation.is_tick_up)) {
                ...
                if (final_score !== null && Math.abs(final_score) >= MINIMUM_SCORE_THRESHOLD) {
                    ...
                    // --- MODIFIED: Payload now only sends the trend score under the 'p' key ---
                    const payload = {
                        type: 'F',
                        p: trend_score
                    };
...
console.log(`-- Config: Trend score starts at ${TREND_SCORE_INITIAL}, updates by ${TREND_SCORE_INCREMENT}, range [${TREND_SCORE_MIN}-${TREND_SCORE_MAX}]`);
