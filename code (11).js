...
function connectToBinance() {
                    ...
                    if (immediateTickDirection === "INCREASE") {
                        if (latestBybitBestBid !== null && prevBybitBidAtLastBinanceIncrease !== null) {
                            if (latestBybitBestBid > prevBybitBidAtLastBinanceIncrease) {
                                const signalMsg = { o: "binance", p: extractedBinanceData.p, signal: "BUY" };
                        ...
                        if (latestBybitBestBid !== null && prevBybitBidAtLastBinanceDecrease !== null) {
                            if (latestBybitBestBid < prevBybitBidAtLastBinanceDecrease) {
                                const signalMsg = { o: "binance", p: extractedBinanceData.p, signal: "SELL" };
                ...
                if (shouldSendData) {
                    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                        try {
                            ...
                            // console.log(`[DATA] Sent Binance general update: ${extractedBinanceData.p}`); 
                        } catch (sendError) {
    ...
    binanceWsClient.on('pong', () => { /* console.log('[Listener] Pong received from Binance.'); */ });
...
function connectToBybit() {
    ...
    bybitWsClient.on('open', function open() {
        ...
        try {
            ...
            // console.log(`[Listener] PID: ${process.pid} --- Bybit: Sent subscription request.`);
        } catch (e) {
    ...
    bybitWsClient.on('message', function incoming(data) {
        try {
            ...
            if (parsedMessage.op === 'ping' || (parsedMessage.ret_msg === 'pong' && parsedMessage.op === 'ping')) {
                // console.log(`[Listener] Bybit ping/pong: ${messageString.substring(0,100)}`); 
                return;
            }
                ...
                if (shouldSendData) {
                    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                        try {
                            ...
                            // console.log(`[DATA] Sent Bybit general update: ${generalBybitPayload.p}`); 
                        } catch (sendError) {
    ...
    bybitWsClient.on('close', function close(code, reason) {
        ...
        // console.log(`[SIG] Bybit disconnected. Resetting Bybit signal baselines.`); // Redundant with close log
...
console.log(`[Listener] Ticker Direction signal logic enabled. Client payload: {o, p, signal?}.`);
...
// console.log(`[Listener] PID: ${process.pid} --- Initial connection attempts initiated.`); // Individual connect logs are better