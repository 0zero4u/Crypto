...
function connectToBinanceStream() {
    ...
    binanceWsClient.on('open', () => console.log('[Predictor] [INFO] [Binance WS] Connection established. Awaiting subscription confirmation...'));

    binanceWsClient.on('message', (data) => {
        try {
            const eventData = JSON.parse(data.toString());

            // --- CORRECTED LOGIC: Handle different message types ---

            // 1. Handle Subscription Confirmation
            if (eventData.result === null && eventData.id !== undefined) {
                console.log(`[Predictor] [INFO] [Binance WS] Subscription confirmation received. Ready for market data.`);
                return; // This is a confirmation message, not data. Stop processing.
            }

            // 2. Handle Book Ticker Data
            else if (eventData.e === 'bookTicker') {
                // console.log(`[Predictor] [DEBUG] [Binance WS] Received BBO: BidP=${eventData.b} | AskP=${eventData.a}`);
            ...
            // 3. Handle any other unexpected messages
            else {
                console.warn(`[Predictor] [WARN] [Binance WS] Received unhandled message type. Data: ${JSON.stringify(eventData)}`);
            }
