...
// =================================================================
// === START: CORRECTED DATA PROCESSING FUNCTION ===================
// =================================================================
function processCoinDCXTradeUpdate(tradeEvent) {
    // 1. Check if the main event object and its 'data' property exist
    if (!tradeEvent || !tradeEvent.data) return;

    try {
        // 2. The 'data' property is a STRING, so we must parse it into a JSON object first.
        const tradeData = JSON.parse(tradeEvent.data);

        // 3. Now we can access the 'p' (price) property from the parsed object.
        if (tradeData.p) {
...
// =================================================================
// === END: CORRECTED DATA PROCESSING FUNCTION =====================
// =================================================================
...
function sendToInternalClient(payload) {
    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
        try {
            // Log the data just before sending it to the receiver
            console.log(`[Listener] --> Sending to internal receiver: ${JSON.stringify(payload)}`);
