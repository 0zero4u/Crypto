...
// --- Formula Weights & Normalizers (TUNED FOR BTCUSDT) ---
const MODEL_VERSION = "2.2-tuned";
const WINDOW_SIZES = [5, 20];
const SPREAD_NORMALIZER = 0.10; // TUNED: Reflects tighter typical spreads.
const BASELINE_WINDOW_SECONDS = 300;
const FREQ_MULTIPLIER = 3.0;
const FREQ_FLOOR_TPS = 15.0; // TUNED: Reflects higher base activity of BTCUSDT.

const WEIGHTS = {
    wmp_mid_diff: 3.0,          // TUNED: Increased weight for most direct price pressure.
    spread_normalized: -1.0,
    tvi_5: 1.0,
    delta_tvi_5: 2.5,           // TUNED: Increased weight for immediate momentum.
    freq_norm_5: 0.5,
    tvi_20: 1.0,                // TUNED: Decreased weight for slower context.
    delta_tvi_20: 1.0,
    freq_norm_20: 0.3
};
...
function processTrade(data) {
    try {
        ...
        const finalPayload = createFinalPayload(data, score, flowFeatures, baselineFreq);
...
function createFinalPayload(tradeData, score, flowFeatures, baselineFreq) {
    let bias = "NEUTRAL";
    if (score > 2.5) bias = "STRONG_BUY_BIAS";
    else if (score > 1.0) bias = "WEAK_BUY_BIAS";
    else if (score < -2.5) bias = "STRONG_SELL_BIAS";
    else if (score < -1.0) bias = "WEAK_SELL_BIAS";
