// --- START OF FILE predictive_listener_robust_v5.js ---
...
const WEIGHT_VWAP_PROGRESSION = 0.30; // Renamed for clarity
...
const URGENCY_TRADE_HISTORY_SIZE = 15;
const VWAP_CALCULATION_WINDOW = 20; // Calculate VWAP over the last 20 trades
...
let state = {
    ...
    recentTradesForVWAP: [], // Stores {price, qty} for VWAP
    participationScore: 0,      // Weak Urgency Score
    progressionScore: 0,        // Strong Urgency Score (now based on VWAP)
};
...
function processTrade(trade) {
    ...
    if (state.recentTradesForVWAP.length < VWAP_CALCULATION_WINDOW || Math.abs(imbalancePercent) < MIN_IMBALANCE_FOR_URGENCY) {
        state.progressionScore *= 0.75; // Decay if conditions aren't met
        return;
    }
...
console.log(`[Predictor] Starting up ROBUST Model V5 for symbol: ${SYMBOL}...`);
