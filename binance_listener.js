...
function cleanupAndExit(exitCode = 1) {
    ...
    const intervalsToClear = [processingIntervalId, pingIntervalId];
...
function connectToBybitStream() {
    ...
    bybitWsClient.on('open', function open() {
        ...
        processingIntervalId = setInterval(() => {
            if (latestOrderbookData) {
                processOrderbookUpdate(latestOrderbookData);
                latestOrderbookData = null; 
            }
    ...
    bybitWsClient.on('message', function incoming(data) {
        try {
            ...
            if (parsedData) {
                latestOrderbookData = parsedData;
            }
        } catch (e) {
...
console.log(`[Listener] PID: ${process.pid} --- Bybit Listener started for ${SYMBOL} (Best Bid). Throttling checks to every ${PROCESSING_INTERVAL_MS}ms. Filtering on >= ${PRICE_CHANGE_THRESHOLD} price change.`); // Keep: Critical Startup Info
