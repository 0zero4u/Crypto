...
let lastSentSignalType = null; // NEW: Stores "BUY" or "SELL" of the last sent signal
...
function connectToInternalReceiver() {
    ...
    internalWsClient.on('open', () => {
        ...
        lastSentSignalType = null; // Reset on new/reconnect to internal client
    });
    ...
    internalWsClient.on('close', (code, reason) => {
        ...
        // lastSentSignalType is not reset here, only on 'open'
                    ...
                    if (immediateTickDirection === "INCREASE") {
                        if (latestBybitBestBid !== null && prevBybitBidAtLastBinanceIncrease !== null) {
                            if (latestBybitBestBid > prevBybitBidAtLastBinanceIncrease) { // Bybit confirms
                                ...
                                if (lastSentSignalType !== currentSignalType) { // Not a consecutive duplicate
                                    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                                        ...
                                        try {
                                            ...
                                            lastSentSignalType = currentSignalType; // Update only if sent successfully
                                        } catch (e) {
...
console.log(`[Listener] Ticker Direction signal logic enabled. Client payload: {o, p, signal?}. Consecutive identical signals are suppressed.`); // Updated log
