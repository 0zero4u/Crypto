...
const MINIMUM_SCORE_THRESHOLD = 75;  
const MONITORING_WINDOW_MS = 100;

// --- Predictive Model Configuration ---
const MICRO_PRICE_SIGNAL_THRESHOLD = 0.001;
// --- NEW: Combined Score Configuration ---
const MICRO_PRICE_NORMALIZATION_CAP = 0.004; // The m_sig value that maps to a score of 100. Should be > threshold.
const WEIGHT_IMBALANCE = 0.5; // Contribution from the volume imbalance score.
const WEIGHT_MICRO_PRICE = 0.5; // Contribution from the micro-price score.
...
// --- NEW: Combined Score Calculation ---
function calculateCombinedScore(imbalanceScore, microPriceSignal) {
    if (imbalanceScore === null || microPriceSignal === null) return null;

    // 1. Normalize the micro-price signal to a [-100, 100] scale
    let normalizedMicroPriceScore = (microPriceSignal / MICRO_PRICE_NORMALIZATION_CAP) * 100;
    // Clamp the value to the [-100, 100] range
    normalizedMicroPriceScore = Math.max(-100, Math.min(100, normalizedMicroPriceScore));

    // 2. Calculate the weighted average of the two scores
    const combined = (imbalanceScore * WEIGHT_IMBALANCE) + (normalizedMicroPriceScore * WEIGHT_MICRO_PRICE);
    ...
    spotWsClient.on('message', (data) => {
        try {
            ...
            if (pending_futures_confirmation && isSpotImbalanceFavorable(pending_futures_confirmation.is_tick_up)) {
                // *** MODIFIED SECTION: Calculate all signals and the combined score ***
                ...
                const combined_score = calculateCombinedScore(imbalance_score, micro_price_data?.signal);

                // *** HIGH-CONVICTION FILTER (based on raw imbalance score) ***
                if (imbalance_score !== null && Math.abs(imbalance_score) >= MINIMUM_SCORE_THRESHOLD) {
...
console.log(`-- Config: Signal requires imbalance score magnitude >= ${MINIMUM_SCORE_THRESHOLD}`);
console.log(`-- Config: Micro-price threshold = ${MICRO_PRICE_SIGNAL_THRESHOLD}, normalization cap = ${MICRO_PRICE_NORMALIZATION_CAP}`);
console.log(`-- Config: Combined score weights: Imbalance=${WEIGHT_IMBALANCE}, MicroPrice=${WEIGHT_MICRO_PRICE}`);
