...
const SIGNAL_THRESHOLD = 0.001; // Predict UP if Y_est > +0.001, DOWN if Y_est < -0.001
const HYSTERESIS_BUFFER = 0.0002; // A buffer to prevent signal flickering
...
let state = {
    ...
    bestAskPrice: 0,
    bestAskQty: 0,
    lastBestBidPrice: 0, // To track bid price changes for "Tick V"
};
...
function calculateAndSendSignal() {
    ...
    // Ensure we have valid data and liquidity to prevent division by zero
    ...
    binanceWsClient.on('message', (data) => {
        try {
            ...
            if (eventData.e === 'bookTicker') {
                ...
                // --- START: Tick Change Detection Logic for "Tick V" ---
                ...
                // --- END: Tick Change Detection Logic ---
