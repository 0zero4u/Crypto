...
const SIGNAL_THRESHOLD = 0.00010; // This is the baseline for a "WEAK" signal (where score = 1.0)
...
function calculateAndSendSignal() {
    ...
    // Define the thresholds for weak and strong signals based on the combined score
    const WEAK_BUY_THRESHOLD = 1.0;
    const STRONG_BUY_THRESHOLD = WEAK_BUY_THRESHOLD * STRONG_SIGNAL_THRESHOLD_MULTIPLIER; // e.g., 2.0
    const WEAK_SELL_THRESHOLD = -1.0;
    const STRONG_SELL_THRESHOLD = WEAK_SELL_THRESHOLD * STRONG_SIGNAL_THRESHOLD_MULTIPLIER; // e.g., -2.0

    let newSignal = lastSentSignal;

    // Determine the potential new signal based on the current score
    ...
    if (lastSentSignal === 'NEUTRAL') {
        // If we are neutral, any valid signal can be adopted
        newSignal = potentialSignal;
    } else if (lastSentSignal.includes('BUY')) {
        // If we were in a BUY state...
        // ...and the score drops enough, switch to NEUTRAL
        if (combinedScore < (WEAK_BUY_THRESHOLD - (HYSTERESIS_BUFFER / SIGNAL_THRESHOLD))) {
            newSignal = 'NEUTRAL';
        } else {
            // ...otherwise, update to the current potential signal (e.g., WEAK_BUY -> STRONG_BUY)
            newSignal = potentialSignal;
        }
    } else if (lastSentSignal.includes('SELL')) {
        // If we were in a SELL state...
        // ...and the score rises enough, switch to NEUTRAL
        if (combinedScore > (WEAK_SELL_THRESHOLD + (HYSTERESIS_BUFFER / SIGNAL_THRESHOLD))) {
            newSignal = 'NEUTRAL';
        } else {
            // ...otherwise, update to the current potential signal (e.g., WEAK_SELL -> STRONG_SELL)
            newSignal = potentialSignal;
        }
    }
    ...
    binanceWsClient.on('message', (data) => {
        try {
            ...
            if (eventData.b && eventData.a) { // Standard book ticker message
                ...
                // CRITICAL: After calculation, update "last" state values for the next event.
                // This must happen AFTER calculateAndSendSignal to ensure the delta is
                // calculated between the previous and current state, not the current and current state.
