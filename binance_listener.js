...
const MODEL_VERSION = "2.4-strict-filter";
...
// *** TUNED: Stricter threshold to reduce signal frequency ***
...
function createFinalPayload(tradeData, score, bias, flowFeatures, baselineFreq) {
    return { type: "prediction_score", timestamp_utc: new Date(tradeData.E).toISOString(), event_timestamp_exchange: tradeData.E, model_version: MODEL_VERSION, prediction: { score: parseFloat(score.toFixed(4)), bias: bias, }, features: { liquidity_pressure: { source_timestamp: lastBookFeatures.source_timestamp, wmp_mid_diff: parseFloat(lastBookFeatures.wmp_mid_diff.toFixed(4)), spread_normalized: parseFloat(lastBookFeatures.spread_normalized.toFixed(4)), }, trade_flow_short_term: { window_size: 5, tvi: parseFloat(flowFeatures[5].tvi.toFixed(4)), delta_tvi: parseFloat(flowFeatures[5].delta_tvi.toFixed(4)), frequency_normalized: parseFloat(flowFeatures[5].freq_norm.toFixed(4)), }, trade_flow_medium_term: { window_size: 20, tvi: parseFloat(flowFeatures[20].tvi.toFixed(4)), delta_tvi: parseFloat(flowFeatures[20].delta_tvi.toFixed(4)), frequency_normalized: parseFloat(flowFeatures[20].freq_norm.toFixed(4)), }, market_context: { baseline_freq_5min: parseFloat(baselineFreq.toFixed(2)), } } };
}
function sendToInternalClient(payload) {
    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) { try { internalWsClient.send(JSON.stringify(payload)); } catch (sendError) { console.error(`[Predictor] Error sending data to internal receiver: ${sendError.message}`); } }
}
