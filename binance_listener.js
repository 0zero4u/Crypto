// --- START OF FILE binance_listener.js (with KuCoin Debugging) ---
...
function processKucoinPrice(price) {
    console.log(`[KuCoin Debug] Processing new price: ${price}`);
    ...
    if (shouldSend) {
        console.log(`[KuCoin Debug] Threshold met. Sending price ${price} to internal client.`);
...
function calculateAndSendAverage() {
    ...
    if (last_sent_avg_price === null) {
        ...
        if (priceDifference >= AVG_PRICE_CHANGE_THRESHOLD) shouldSend = true;
    }
    if (shouldSend) {
...
// --- Exchange Connection Functions (Binance, Bybit, OKX - unchanged) ---
function connectToBinance() { binanceWsClient = new WebSocket(BINANCE_SPOT_STREAM_URL); binanceWsClient.on('open', () => { latestBinanceBid = null; last_sent_avg_price = null; }); binanceWsClient.on('message', (data) => { try { const message = JSON.parse(data.toString()); if (message.b) { processNewPriceUpdate('binance', parseFloat(message.b)); } } catch (e) {} }); binanceWsClient.on('error', () => {}); binanceWsClient.on('close', () => { binanceWsClient = null; setTimeout(connectToBinance, RECONNECT_INTERVAL_MS); }); }
function connectToBybit() { bybitWsClient = new WebSocket(BYBIT_SPOT_STREAM_URL); if (bybitHeartbeatInterval) clearInterval(bybitHeartbeatInterval); bybitWsClient.on('open', () => { latestBybitBid = null; last_sent_avg_price = null; bybitWsClient.send(BYBIT_SUBSCRIBE_MSG); bybitHeartbeatInterval = setInterval(() => { if (bybitWsClient.readyState === WebSocket.OPEN) { bybitWsClient.send(BYBIT_PING_MSG); } }, 20000); }); bybitWsClient.on('message', (data) => { try { const message = JSON.parse(data.toString()); if (message.topic && message.topic.startsWith('orderbook.1') && message.data) { const bestBid = message.data.b?.[0]?.[0]; if (bestBid) { processNewPriceUpdate('bybit', parseFloat(bestBid)); } } } catch (e) {} }); bybitWsClient.on('error', () => {}); bybitWsClient.on('close', () => { bybitWsClient = null; if (bybitHeartbeatInterval) clearInterval(bybitHeartbeatInterval); setTimeout(connectToBybit, RECONNECT_INTERVAL_MS); }); }
function connectToOkx() { okxWsClient = new WebSocket(OKX_STREAM_URL); okxWsClient.on('open', () => { latestOkxBid = null; last_sent_avg_price = null; okxWsClient.send(OKX_SUBSCRIBE_MSG); }); okxWsClient.on('message', (data) => { const messageString = data.toString(); if (messageString === 'ping') { if (okxWsClient.readyState === WebSocket.OPEN) { okxWsClient.send('pong'); } return; } try { const message = JSON.parse(messageString); if (message.arg?.channel === 'bbo-tbt' && message.data?.[0]) { const bestBid = message.data[0].bids?.[0]?.[0]; if (bestBid) { processNewPriceUpdate('okx', parseFloat(bestBid)); } } } catch (e) {} }); okxWsClient.on('error', () => {}); okxWsClient.on('close', () => { okxWsClient = null; setTimeout(connectToOkx, RECONNECT_INTERVAL_MS); }); }


// --- KuCoin Connection with Debug Logging ---
function connectToKucoin() {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            console.log('[KuCoin Debug] Received token response from server:', data);
            try {
                const response = JSON.parse(data);
                if (response.code !== '200000' || !response.data.token) {
                    console.error('[KuCoin Debug] Failed to get a valid token. Response:', response);
                ...
                console.log('[KuCoin Debug] Successfully got token. Connecting to WebSocket endpoint:', endpoint);
                establishKucoinWsConnection(`${endpoint}?token=${token}`, pingInterval);
            } catch (e) {
                console.error('[KuCoin Debug] Error parsing token JSON response. Error:', e.message);
                setTimeout(connectToKucoin, RECONNECT_INTERVAL_MS);
            }
        });
    });

    req.on('error', (e) => { // *** ADDED LOGGING FOR REQUEST ERROR ***
        console.error('[KuCoin Debug] HTTPS request for token failed. Error:', e.message);
        setTimeout(connectToKucoin, RECONNECT_INTERVAL_MS);
    });
...
function establishKucoinWsConnection(url, pingInterval) {
    ...
    console.log('[KuCoin Debug] Establishing WebSocket connection to:', url);
    ...
    kucoinWsClient.on('message', (data) => {
        ...
        try {
            ...
            if (message.type === 'message' && message.topic === `/market/level1:${KUCOIN_SYMBOL}`) {
                ...
                console.log('[KuCoin Debug] Received welcome message. Connection ID:', message.id);
            } else if (message.type === 'pong') {
                // Pong received, this is good. No need to log every time unless debugging heartbeat.
            }
        } catch (e) {
            console.error('[KuCoin Debug] Error parsing message from server. Error:', e.message);
        }
    });

    kucoinWsClient.on('error', (err) => { // *** ADDED LOGGING FOR WEBSOCKET ERROR ***
        console.error('[KuCoin Debug] WebSocket connection error:', err.message);
    });

    kucoinWsClient.on('close', (code, reason) => { // *** ADDED LOGGING FOR WEBSOCKET CLOSE ***
...
// --- END OF FILE binance_listener.js (with KuCoin Debugging) ---
