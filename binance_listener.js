...
const SCORE_UPDATE_DEBOUNCE_MS = 100; // Wait 100ms after last score update to send

// --- Formula Weights, Normalizers & Filtering Parameters ---
const MODEL_VERSION = "2.8-debounced"; // Updated model version
...
let scoreUpdateDebounceTimer = null; // Timer for debouncing score updates


// --- initializeState, connectToInternalReceiver, processDepthUpdate ---
function initializeState() {
    ...
    if (scoreUpdateDebounceTimer) clearTimeout(scoreUpdateDebounceTimer);
...
// --- updateTVI, updateTradeFreq, updateBaseline, getBiasFromScore are unchanged ---
...
// *** MODIFIED FUNCTION: Intelligent Debouncing ***
function processTrade(data) {
    try {
        ...
        if (biasChanged) {
            // Rule 1: Bias changed, a CRITICAL event. Send immediately.
        ...
        } else if (scoreChangedSignificantly) {
            // Rule 2: Bias is the same, but score moved. Debounce the update.
            if (scoreUpdateDebounceTimer) {
                clearTimeout(scoreUpdateDebounceTimer);
            }
            scoreUpdateDebounceTimer = setTimeout(() => {
                ...
                lastSentPrediction.score = score;
                lastSentPrediction.bias = currentBias;
                scoreUpdateDebounceTimer = null; // Clear timer reference
...
console.log(`[Predictor] PID: ${process.pid} --- Predictive Indicator Started for ${SYMBOL} with debouncing logic.`);
