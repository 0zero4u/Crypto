...
const SIGNAL_THRESHOLD = 0.001; // This now defines a "1.0" score.
...
function calculateAndSendSignal() {
    ...
    // --- SCORE SYSTEM TWIST ---
    // Normalize the signal value against the threshold to get a score.
    // A score of 1.0 means the pressure is strong enough to trigger a BUY.
    // A score of -1.0 means the pressure is strong enough to trigger a SELL.
    const score = signalValue / SIGNAL_THRESHOLD;
    // Cap the score at +/- 1.0 for a clean -100% to +100% range.
    const cappedScore = Math.max(-1.0, Math.min(1.0, score));
    ...
    if (newSignal !== lastSentSignal) {
        const payload = {
            sig: newSignal,
            score: parseFloat(cappedScore.toFixed(4)), // The new normalized score
    ...
    binanceWsClient.on('message', (data) => {
        try {
            ...
            if (eventData.b && eventData.a) {
                ...
                // Calculate and potentially send a signal on EVERY update.
