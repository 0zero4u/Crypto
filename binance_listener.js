...
const PRICE_CHANGE_THRESHOLD = 1.0; // For general price updates, NOT for tick signals
...
function connectToInternalReceiver() {
    if (internalWsClient && (internalWsClient.readyState === WebSocket.OPEN || internalWsClient.readyState === WebSocket.CONNECTING)) {
        console.log(`[Listener] Internal receiver connection attempt skipped: already open or connecting. State: ${internalWsClient.readyState}`);
        return;
    }
...
function manualExtractBinanceAggTradeData(messageString) {
    try {
        ...
        // Silent parse error, will be handled by null check
        return null;
    }
}
...
function connectToBinance() {
    if (binanceWsClient && (binanceWsClient.readyState === WebSocket.OPEN || binanceWsClient.readyState === WebSocket.CONNECTING)) {
        console.log(`[Listener] Binance connection attempt skipped: already open or connecting. State: ${binanceWsClient.readyState}`);
        return;
    }
        ...
        binancePingIntervalId = setInterval(() => {
            if (binanceWsClient && binanceWsClient.readyState === WebSocket.OPEN) {
                try {
                    ...
                    console.error(`[Listener] PID: ${process.pid} --- Binance: Error sending ping:`, pingError.message);
                }
            }
    ...
    binanceWsClient.on('message', function incoming(data) {
        try {
            ...
            const generalBinanceData = manualExtractBinanceAggTradeData(messageString); 

            if (generalBinanceData) {
                const currentPrice = parseFloat(generalBinanceData.p);
                if (isNaN(currentPrice)) {
                    // This warning is important if data format changes or parsing fails unexpectedly
                    console.warn(`[Listener] PID: ${process.pid} --- Binance: Invalid price in aggTrade data:`, generalBinanceData.p);
                    return;
                }
                ...
                if (lastBinanceAggTradePrice_Direction === null) {
                    ...
                    if (immediateTickDirection === "INCREASE") {
                        if (latestBybitBestBid !== null && prevBybitBidAtLastBinanceIncrease !== null) {
                            if (latestBybitBestBid > prevBybitBidAtLastBinanceIncrease) {
                                const signalMsg = { type: "TICK_SIGNAL", signal: "BUY", timestamp: generalBinanceData.E };
                                console.log(`[SIG] --> Sending TRUE TICK [BUY]. Binance: ${lastBinanceAggTradePrice_Direction.toFixed(2)}->${currentPrice.toFixed(2)}, Bybit Bid: ${prevBybitBidAtLastBinanceIncrease.toFixed(2)}->${latestBybitBestBid.toFixed(2)}`);
                                if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                                    try { internalWsClient.send(JSON.stringify(signalMsg)); }
                                    catch (e) { console.error(`[Listener] PID: ${process.pid} --- Error sending TRUE TICK [BUY]:`, e.message); }
                                }
                            } // else: False Tick [BUY] - No log, no client message
                        }
                        ...
                        if (latestBybitBestBid !== null && prevBybitBidAtLastBinanceDecrease !== null) {
                            if (latestBybitBestBid < prevBybitBidAtLastBinanceDecrease) {
                                const signalMsg = { type: "TICK_SIGNAL", signal: "SELL", timestamp: generalBinanceData.E };
                                console.log(`[SIG] --> Sending TRUE TICK [SELL]. Binance: ${lastBinanceAggTradePrice_Direction.toFixed(2)}->${currentPrice.toFixed(2)}, Bybit Bid: ${prevBybitBidAtLastBinanceDecrease.toFixed(2)}->${latestBybitBestBid.toFixed(2)}`);
                                if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                                    try { internalWsClient.send(JSON.stringify(signalMsg)); }
                                    catch (e) { console.error(`[Listener] PID: ${process.pid} --- Error sending TRUE TICK [SELL]:`, e.message); }
                                }
                            } // else: False Tick [SELL] - No log, no client message
                        }
                ...
                // --- General Price Update Sending (based on PRICE_CHANGE_THRESHOLD) ---
                ...
                if (lastSentBinancePrice === null) shouldSendData = true;
                    ...
                    if (priceDifference >= PRICE_CHANGE_THRESHOLD) shouldSendData = true;
                }

                if (shouldSendData) {
                    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                        try {
                            generalBinanceData.source = "binance";
                            internalWsClient.send(JSON.stringify(generalBinanceData)); 
                            lastSentBinancePrice = currentPrice;
                            // console.log(`[DATA] Sent Binance general update: ${generalBinanceData.p}`); // Optional: log if general data is sent
                        } catch (sendError) {
                ...
                // This block handles non-aggTrade messages or parsing failures from manualExtract
                if (messageString && !messageString.includes('"e":"pong"')) { // Binance sends {"e":"pong"} to our pings
                    let isPotentiallyJson = false;
                    try { JSON.parse(messageString); isPotentiallyJson = true; } catch (e) { /* not json */ }

                    if (isPotentiallyJson && !messageString.includes('"e":"aggTrade"')) { // It's JSON but not what we expect
                    ...
                    } else if (!isPotentiallyJson) { // Not JSON at all
                         console.warn(`[Listener] PID: ${process.pid} --- Binance: Received non-JSON message. Snippet:`, messageString.substring(0, 150));
                    }
                    // If manualExtractBinanceAggTradeData returned null for an aggTrade message, it means a parsing issue with expected format.
                }
            }
        } catch (e) {
    ...
    binanceWsClient.on('pong', () => { /* console.log('[Listener] Pong received from Binance.'); // Usually too noisy */ });
...
function connectToBybit() {
    if (bybitWsClient && (bybitWsClient.readyState === WebSocket.OPEN || bybitWsClient.readyState === WebSocket.CONNECTING)) {
        console.log(`[Listener] Bybit connection attempt skipped: already open or connecting. State: ${bybitWsClient.readyState}`);
        return;
    }
    ...
    bybitWsClient.on('open', function open() {
        ...
        try {
            ...
            // console.log(`[Listener] PID: ${process.pid} --- Bybit: Sent subscription request.`); // Optional, success below is better
        } catch (e) {
            console.error(`[Listener] PID: ${process.pid} --- Bybit: Error sending subscription message:`, e.message);
            if (bybitWsClient) bybitWsClient.terminate(); // Force close if sub fails on open
            return;
        }
        ...
        bybitPingIntervalId = setInterval(() => {
            if (bybitWsClient && bybitWsClient.readyState === WebSocket.OPEN) {
                try {
                    bybitWsClient.send(JSON.stringify(BYBIT_PING_PAYLOAD));
                } catch (pingError) {
                    console.error(`[Listener] PID: ${process.pid} --- Bybit: Error sending ping:`, pingError.message);
                }
            }
    ...
    bybitWsClient.on('message', function incoming(data) {
        try {
            ...
            if (parsedMessage.op === 'ping' || (parsedMessage.ret_msg === 'pong' && parsedMessage.op === 'ping')) {
                // console.log(`[Listener] Bybit ping/pong: ${messageString.substring(0,100)}`); // Usually too noisy
                return;
            }
            ...
            if (parsedMessage.topic && parsedMessage.topic === BYBIT_SUBSCRIPTION_PAYLOAD.args[0] && parsedMessage.data) {
                const bybitData = parsedMessage.data;
                let currentBestBidNum = null;

                if (bybitData.b && bybitData.b.length > 0 && bybitData.b[0] && typeof bybitData.b[0][0] === 'string') {
                    currentBestBidNum = parseFloat(bybitData.b[0][0]);
                ...
                if (bybitData.a && bybitData.a.length > 0 && bybitData.a[0] && typeof bybitData.a[0][0] === 'string') priceStringForSending = bybitData.a[0][0]; 
                else if (bybitData.b && bybitData.b.length > 0 && bybitData.b[0] && typeof bybitData.b[0][0] === 'string') priceStringForSending = bybitData.b[0][0]; 

                if (!priceStringForSending) return; // No price to send for general update
                ...
                const timestamp = parsedMessage.ts;
                const generalBybitData = { e: "trade", E: timestamp, p: priceStringForSending, source: "bybit" };
                ...
                if (shouldSendData) {
                    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                        try {
                            internalWsClient.send(JSON.stringify(generalBybitData));
                            lastSentBybitPrice = currentPriceForSending;
                            // console.log(`[DATA] Sent Bybit general update: ${generalBybitData.p}`); // Optional: log if general data is sent
                        } catch (sendError) {
                ...
                // Ignore kline or other subscribed topics if not processed for general updates
                 ...
                 // Filter out common non-error operational messages from Bybit like heartbeats or initial snapshots for orderbook if not explicitly handled
             ...
             if (e instanceof SyntaxError && data.toString().includes("pong")) {
                // Likely Bybit's plain text "pong" if not caught by op code check, can ignore
            } else {
    ...
    bybitWsClient.on('close', function close(code, reason) {
        ...
        console.log(`[SIG] Bybit disconnected. Resetting Bybit signal baselines.`);
...
console.log(`[Listener] General Price Update Threshold: ${PRICE_CHANGE_THRESHOLD}`);
console.log(`[Listener] Ticker Direction signal logic enabled. TRUE TICK signals sent to client.`);
console.log(`[Listener] General price updates include 'source' field ('binance' or 'bybit').`);
