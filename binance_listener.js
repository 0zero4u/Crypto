...
// --- Formula Weights & Normalizers (FINAL) ---
...
// *** MODIFIED STATE: We only need to track the last sent bias ***
let lastSentBias = 'NEUTRAL';
...
function initializeState() {
    ...
    // ... (rest of function body from previous version)
...
// --- connectToInternalReceiver, processDepthUpdate etc. are unchanged ---
...
// This function determines the new bias based on the current score AND the previous bias.
function getNewBias(score, lastBias) {
    switch (lastBias) {
        case 'NEUTRAL':
        ...
        case 'WEAK_BUY_BIAS':
            if (score > 3.0) return 'STRONG_BUY_BIAS';
            if (score < 1.2) return 'NEUTRAL'; // Relaxation threshold
            return 'WEAK_BUY_BIAS';

        case 'STRONG_BUY_BIAS':
            if (score < 2.7) return 'WEAK_BUY_BIAS'; // Relaxation threshold
            return 'STRONG_BUY_BIAS';
            
        case 'WEAK_SELL_BIAS':
            if (score < -3.0) return 'STRONG_SELL_BIAS';
            if (score > -1.2) return 'NEUTRAL'; // Relaxation threshold
            return 'WEAK_SELL_BIAS';
            
        case 'STRONG_SELL_BIAS':
            if (score > -2.7) return 'WEAK_SELL_BIAS'; // Relaxation threshold
...
// *** MODIFIED processTrade with the new hysteresis logic ***
function processTrade(data) {
    try {
        ...
        // --- NEW HYSTERESIS FILTERING LOGIC ---
        const newBias = getNewBias(score, lastSentBias);

        // Only send an update if the bias has officially changed state
...
// --- Full bodies of unchanged functions for completeness ---
