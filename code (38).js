// binance_listener.js (Combined BookTicker & AggTrade Logic - Simplified Payload & Further Reduced Logging - Dynamic AggTrade Confirmation - Filtered Bid Price Push)
...
function cleanupAndExit(exitCode = 1) {
    console.log(`[Listener] PID: ${process.pid} --- Initiating cleanup...`); // Keep - Essential for shutdown lifecycle
    ...
    clientsToTerminate.forEach(client => {
        if (client && typeof client.terminate === 'function') {
            try { client.terminate(); } catch (e) { console.error(`[Listener] Error terminating a WebSocket client: ${e.message}`); } // Keep - Error during cleanup
        }
    });
    ...
    setTimeout(() => {
        console.log(`[Listener] PID: ${process.pid} --- Exiting with code ${exitCode}.`); // Keep - Essential for shutdown lifecycle
...
const SYMBOL = 'btcusdt'; // Ensure SYMBOL is lowercase
...
console.log(`[Listener] PID: ${process.pid} --- Binance Listener (Dynamic AggTrade, Filtered Bid) started for ${SYMBOL}.`);
// Parameter logs removed for brevity
...
// console.log(`[Listener] Filtered Best Bid Push: Threshold=${BID_PRICE_FILTER_THRESHOLD}`);