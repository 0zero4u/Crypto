...
const DESIRED_PROFIT_THRESHOLD_USD = 60.0;
...
// --- Arbitrage Logic Core (Called by Interval) --- // *** UPDATED ***
function performArbitrageCheckAndSignal() {
    ...
    // --- Calculate potential profits based on executable "basis" for each direction ---

    // Scenario 1: Sell Futures, Buy Spot (Contango Arbitrage)
    // "Basis" here is (Futures Sell Price) - (Spot Buy Price)
    // We sell futures at Futures Best Bid, Buy spot at Spot Best Ask.
    const grossBasisFuturesSellSpotBuy = latestFuturesData.bestBid - latestSpotData.bestAsk;
    const netProfitFuturesSellSpotBuy = grossBasisFuturesSellSpotBuy - TOTAL_FEES_PER_UNIT_USD;

    // Scenario 2: Sell Spot, Buy Futures (Backwardation Arbitrage)
    // "Basis" here is (Spot Sell Price) - (Futures Buy Price)
    // We sell spot at Spot Best Bid, Buy futures at Futures Best Ask.
    const grossBasisSpotSellFuturesBuy = latestSpotData.bestBid - latestFuturesData.bestAsk;
    const netProfitSpotSellFuturesBuy = grossBasisSpotSellFuturesBuy - TOTAL_FEES_PER_UNIT_USD;


    // --- Determine if an arbitrage opportunity exists ---

    // Check Scenario 1 (Sell Futures, Buy Spot) first
    if (netProfitFuturesSellSpotBuy > DESIRED_PROFIT_THRESHOLD_USD) {
        currentOpportunityType = "scenario1_SellFuturesBuySpot";
        identifiedProfit = netProfitFuturesSellSpotBuy;
    ...
    // Else, check Scenario 2 (Sell Spot, Buy Futures)
    else if (netProfitSpotSellFuturesBuy > DESIRED_PROFIT_THRESHOLD_USD) {
        currentOpportunityType = "scenario2_SellSpotBuyFutures";
        identifiedProfit = netProfitSpotSellFuturesBuy;
        ...
        if (currentOpportunityType !== lastSentArbitrageType) {
            if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
                try {
                    ...
                    console.log(`[Arbitrage] PID: ${process.pid} --- SENT Signal. Type: ${currentOpportunityType}, Sell: ${arbSignalPayload.arb_signal.sell_market} (SpotP: ${arbSignalPayload.arb_signal.spot_price_trade}, FuturesP: ${arbSignalPayload.arb_signal.futures_price_trade}), Buy: ${arbSignalPayload.arb_signal.buy_market}, Profit: $${identifiedProfit.toFixed(4)}`);
