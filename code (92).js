// --- START OF FILE predictive_listener_robust_v5-final-concise.js ---
...
const URGENCY_TRADE_HISTORY_SIZE = 4;
const VWAP_CALCULATION_WINDOW = 4;
...
// --- MODIFIED --- This function now creates the hyper-concise payload
function calculateAndSendPredictScore() {
    ...
    if (newSignal !== lastSentSignal) {
        let signalCode = 0; // Neutral
        if (newSignal === 'Strong Buy') signalCode = 1;
        else if (newSignal === 'Strong Sell') signalCode = -1;

        // Create the hyper-concise payload as requested
        const payload = {
            sig: signalCode,
            sc: parseFloat(score.toFixed(2)),
            imb: parseFloat(imbalancePercent.toFixed(2)),
            prog: parseFloat(progressionScore.toFixed(2)),
            momentum: parseFloat(imbalanceMomentum.toFixed(2)),
        };
        
        sendToInternalClient(payload);
        lastSentSignal = newSignal;
        // Log the human-readable version for server-side debugging
        console.log(`[Predictor] SIGNAL CHANGE: ${newSignal.padEnd(12)}| Sent: ${JSON.stringify(payload)}`);
    }
}
...
console.log(`[Predictor] Starting up ROBUST Model V5-Final-Concise (4-trade window) for symbol: ${SYMBOL}...`);
