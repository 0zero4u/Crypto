...
function calculateAndSendSignal() {
    ...
    if (totalVolume === 0 || bestBidPrice <= 0 || bestAskPrice <= 0) {
        return; // Silently skip if data is invalid, as BBO can be momentarily empty
    }
    ...
    if (newSignal !== lastSentSignal) {
        console.log(`[Predictor] [INFO] [Signal Logic] SIGNAL STATE CHANGE: ${lastSentSignal} -> ${newSignal} (Y_est: ${signalValue.toFixed(6)})`);
        const payload = {
            sig: newSignal,
...
function connectToBinanceStream() {
    ...
    binanceWsClient.on('message', (data) => {
        try {
            ...
            // 2. Handle Book Ticker Data (check for expected keys instead of 'e')
            ...
            // 3. Handle any other unexpected messages
                ...
                console.warn(`[Predictor] [WARN] [Binance WS] Received unhandled message format. Data: ${JSON.stringify(eventData)}`);
            }
