...
const WEIGHT_IMBALANCE = 0.5;
const WEIGHT_MICRO_PRICE = 0.5;
// --- NEW: Final signal filter is now based on the combined score ---
    ...
    spotWsClient.on('message', (data) => {
        try {
            ...
            if (pending_futures_confirmation && isSpotImbalanceFavorable(pending_futures_confirmation.is_tick_up)) {
                ...
                // *** MODIFIED HIGH-CONVICTION FILTER (based on COMBINED score) ***
                // Only proceed if the combined score magnitude is strong enough.
