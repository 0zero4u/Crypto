// --- START OF FILE binance_listener (17).js ---

// --- START OF FILE binance_listener.js ---

const WebSocket = require('ws');

// --- Global Error Handlers (Critical Logs Only) ---
process.on('uncaughtException', (err, origin) => {
    console.error(`[Listener] PID: ${process.pid} --- FATAL: UNCAUGHT EXCEPTION`);
    console.error(err.stack || err);
    console.error(`[Listener] Exception origin: ${origin}`);
    console.error(`[Listener] PID: ${process.pid} --- Exiting due to uncaught exception...`);
    cleanupAndExit(1);
});

process.on('unhandledRejection', (reason, promise) => {
    console.error(`[Listener] PID: ${process.pid} --- FATAL: UNHANDLED PROMISE REJECTION`);
    console.error('[Listener] Unhandled Rejection at:', promise);
    console.error('[Listener] Reason:', reason instanceof Error ? reason.stack : reason);
    console.error(`[Listener] PID: ${process.pid} --- Exiting due to unhandled promise rejection...`);
    cleanupAndExit(1);
});

function cleanupAndExit(exitCode = 1) {
    const clientsToTerminate = [internalWsClient, binanceWsClient];
    clientsToTerminate.forEach(client => {
        if (client && typeof client.terminate === 'function') {
            try { client.terminate(); } catch (e) { /* Silenced */ }
        }
    });

    setTimeout(() => {
        process.exit(exitCode);
    }, 1000).unref();
}

// --- Listener Configuration ---
const SYMBOL = 'BTCUSDT';
const BINANCE_SPOT_STREAM_URL = `wss://stream.binance.com:9443/ws/${SYMBOL.toLowerCase()}@bookTicker`;
const internalReceiverUrl = 'ws://localhost:8082';
const RECONNECT_INTERVAL_MS = 5000;

// --- Listener State Variables ---
let binanceWsClient = null;
let internalWsClient = null;
let last_sent_ask_price = null;

// --- Internal Receiver Connection (Silent) ---
function connectToInternalReceiver() {
    if (internalWsClient && (internalWsClient.readyState === WebSocket.OPEN || internalWsClient.readyState === WebSocket.CONNECTING)) {
        return;
    }
    internalWsClient = new WebSocket(internalReceiverUrl);
    // Errors are silenced as the 'close' event will handle reconnection automatically.
    internalWsClient.on('error', () => {});
    internalWsClient.on('close', () => {
        internalWsClient = null;
        setTimeout(connectToInternalReceiver, RECONNECT_INTERVAL_MS);
    });
}

// --- Data Extraction Functions ---
function extractBinanceBookTickerData(messageString) {
    try {
        const message = JSON.parse(messageString);
        if (message.s && message.a) {
            return {
                price_ask: parseFloat(message.a)
            };
        }
        return null;
    } catch (error) {
        // This is a critical error: if we can't parse data, the listener is broken.
        console.error(`[Listener] CRITICAL: Error parsing Binance JSON: ${error.message}. Data: ${messageString.substring(0,100)}...`);
        return null;
    }
}

// --- Signal Processing ---
function processBinanceBookTickerUpdate(parsedData) {
    if (!parsedData) return;

    const current_ask_price = parsedData.price_ask;

    if (last_sent_ask_price === null || current_ask_price !== last_sent_ask_price) {
        if (current_ask_price > 0) {
            const pricePayload = {
                p: current_ask_price
            };
            sendToInternalClient(pricePayload);
            last_sent_ask_price = current_ask_price;
        }
    }
}

function sendToInternalClient(payload) {
    if (internalWsClient && internalWsClient.readyState === WebSocket.OPEN) {
        try {
            internalWsClient.send(JSON.stringify(payload));
        } catch (sendError) {
            // Silenced. The connection will close and attempt to reconnect if it's broken.
        }
    }
}

// --- Binance Stream Connection (Silent) ---
function connectToBinanceStream() {
    if (binanceWsClient && (binanceWsClient.readyState === WebSocket.OPEN || binanceWsClient.readyState === WebSocket.CONNECTING)) {
        return;
    }
    binanceWsClient = new WebSocket(BINANCE_SPOT_STREAM_URL);

    binanceWsClient.on('open', function open() {
        last_sent_ask_price = null;
    });

    binanceWsClient.on('message', function incoming(data) {
        try {
            const messageString = data.toString();
            const parsedData = extractBinanceBookTickerData(messageString);
            if (parsedData) {
                processBinanceBookTickerUpdate(parsedData);
            }
        } catch (e) {
            // This is a critical error within the message handling logic itself.
            console.error(`[Listener] CRITICAL ERROR in message handler: ${e.message}`, e.stack);
        }
    });

    // Errors are silenced as the 'close' event will handle reconnection automatically.
    binanceWsClient.on('error', () => {});

    binanceWsClient.on('close', function close() {
        binanceWsClient = null;
        setTimeout(connectToBinanceStream, RECONNECT_INTERVAL_MS);
    });
}

// --- Start the connections ---
connectToInternalReceiver();
connectToBinanceStream();

// --- No startup log for silent operation ---
// --- END OF FILE binance_listener.js ---