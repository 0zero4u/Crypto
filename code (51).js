// --- START OF FILE coindcx_listener.js ---
...
// --- CoinDCX Stream Connection (UPDATED) ---
function connectToCoinDCX() {
    ...
    coindcxSocket.on('connect', () => {
        ...
        // ** NEW: START PING MECHANISM ON CONNECT **
        // First, clear any old interval that might exist from a previous connection
        if (pingIntervalId) clearInterval(pingIntervalId);

        // Start a new interval to send a ping every 25 seconds
        pingIntervalId = setInterval(() => {
            if (coindcxSocket && coindcxSocket.connected) {
                coindcxSocket.emit('ping'); // The payload is optional for ping
                console.log('[Listener] --> Sent ping to CoinDCX server');
            }
        }, PING_INTERVAL_MS);
    });

    // Listen for the server's pong response (optional but good for debugging)
    ...
    coindcxSocket.on('disconnect', (reason) => {
        ...
        // ** NEW: STOP PING MECHANISM ON DISCONNECT **
        if (pingIntervalId) {
