...
// --- MODIFIED --- This function now creates the final specified concise payload
function calculateAndSendPredictScore() {
    ...
    if (lastSentSignal === 'NEUTRAL') {
        if (score > PREDICT_SCORE_BUY_THRESHOLD) newSignal = 'BUY';
        else if (score < PREDICT_SCORE_SELL_THRESHOLD) newSignal = 'SELL';
    ...
    if (newSignal !== lastSentSignal) {
        const payload = {
            ...
            imb: parseFloat(imbalancePercent.toFixed(3)),
            part: parseFloat(participationScore.toFixed(3)),
            prog: parseFloat(progressionScore.toFixed(3)),
            mom: parseFloat(imbalanceMomentum.toFixed(3)),
        };
