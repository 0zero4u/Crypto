    ...
    bybitWsClient.on('message', (data) => {
        try {
            ...
            if (message.topic && message.topic.startsWith('orderbook.50') && message.data) {
                ...
                // ** MODIFIED: Reformat the payload for better readability **
                const formatOrderBookSide = (side) => {
                    // Converts [priceStr, sizeStr] to { price: number, quantity: number }
                ...
                // Convert Maps to sorted arrays first
                ...
                // Construct the new, more readable payload
                const payload = { 
                    type: 'S',
