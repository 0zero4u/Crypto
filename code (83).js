...
function processPriceUpdate(price) {
    ...
    // If this is the first price, always send it to establish a baseline.
    if (last_sent_price === null) {
        shouldSend = true;
    } else {
        // For all other prices, check if the change is significant.
    ...
    // If a send is warranted, send only the new price and update the state.
    if (shouldSend) {
