...
// *** MODIFIED FUNCTION with wider bias bands ***
function getBiasFromScore(score) {
    if (score > 3.0) return "STRONG_BUY_BIAS";  // TUNED: Stricter threshold
    if (score > 1.5) return "WEAK_BUY_BIAS";    // TUNED: Stricter threshold
    if (score < -3.0) return "STRONG_SELL_BIAS"; // TUNED: Stricter threshold
    if (score < -1.5) return "WEAK_SELL_BIAS";   // TUNED: Stricter threshold
    return "NEUTRAL";
}
