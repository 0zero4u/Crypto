...
const MODEL_VERSION = "2.8-side-change-filter"; // Updated model version
...
// --- WIDER and Corrected Hysteresis Thresholds for Bias ---
...
// --- Corrected Hysteresis with a Hierarchical State Machine (unchanged) ---
...
// *** NEW: Helper function to classify bias direction ***
function getBiasSide(bias) {
...
function processTrade(data) {
    try {
        ...
        // *** MODIFIED: Logic to filter by bias "side" change ***
        const scoreChangedSignificantly = Math.abs(score - lastSentPrediction.score) > SCORE_CHANGE_THRESHOLD;
        const sideChanged = getBiasSide(currentBias) !== getBiasSide(lastSentPrediction.bias);

        // Send an update IF:
        // 1. The fundamental "side" has changed (e.g., NEUTRAL -> BUY, or BUY -> SELL).
        // OR
        // 2. The bias is active (NOT NEUTRAL) AND the score has changed significantly (intensification).
        const shouldSendUpdate = sideChanged || (scoreChangedSignificantly && currentBias !== 'NEUTRAL');
